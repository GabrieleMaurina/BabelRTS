...
SOURCE_HERE_PATTERN = cmp_re(
    r'\bsource\s*\([A-Za-z:]*\(["\']([A-Za-z0-9-_.]*\/[A-Za-z0-9-_.]+\.[R])["\']\)\)')
...
FUNCTION_CALLING_PATTERN = cmp_re(
    r'\b(?!{})([A-Za-z0-9-_.]+)\s*\('.format('|'.join(KEYWORDS)))

# sum = function() or sum <- function()
FUNCTION_DECLARATION_PATTERN = cmp_re(
    r'\b([A-Za-z0-9-_.]+)\s*[=<-]+\s*function\s*\(')
    ...
    def chek_and_make_file_path(self, match, folder_path):
        ...
        if len(match.split("/")) < 2:
         ...
         # if  test->src file dependcies
            ...
            file = join('/'.join(folder_path.split("/")
                        [:-1]), src_folder, src_file)
