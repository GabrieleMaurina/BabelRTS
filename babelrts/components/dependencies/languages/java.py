...
EXTENDS_PATTERN = cmp_re(
    r'\bextends\s+([\s\S]+?)\s*(?:<\S*>)?\s*(?:{|implements)')
IMPLEMENTS_PATTERN = cmp_re(r'\bimplements\s+([\s\S]+?)\s*(?:{|extends)')
NEW_PATTERN = cmp_re(r'\bnew\s+(\S+?)\s*(?:<\S*>)?\s*\(\s*')
STATIC_PATTERN = cmp_re(
    r'\b((?:[a-z0-9_\.]+\.)?[A-Z][A-Za-z0-9_]+?)\.[A-Za-z_]')
...
class Java(Language):
    def get_extensions_patterns_actions(self):
        return (
            ...
            ExtensionPatternAction(
                'java', IMPORT_STATIC_PATTERN, self.import_static_action),
            # ExtensionPatternAction('java', PACKAGE_PATTERN, self.package_action),
            ExtensionPatternAction(
                'java', EXTENDS_PATTERN, self.multiple_used_classes_action),
            ExtensionPatternAction(
                'java', IMPLEMENTS_PATTERN, self.multiple_used_classes_action),
            ExtensionPatternAction('java', NEW_PATTERN,
                                   self.used_class_action),
            ExtensionPatternAction(
                'java', STATIC_PATTERN, self.used_class_action),
            ExtensionPatternAction(
                'java', ANNOTATION_PATTERN, self.used_class_action),
            ExtensionPatternAction(
                'java', THROWS_PATTERN, self.multiple_used_classes_action),
            ExtensionPatternAction('java', CATCH_PATTERN,
                                   self.multiple_used_classes_action)
        )
    ...
    def multiple_used_classes_action(self, match, file_path, folder_path, content):
        clazzes = (clazz.split('<', 1)[0] for clazz in (v.strip()
                   for v in SPLIT.split(match)) if clazz)
